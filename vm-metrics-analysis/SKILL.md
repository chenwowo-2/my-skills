...
description: Query VictoriaMetrics for node metrics (CPU, memory, disk, network, load) and optionally OpenStack, Ceph, Keepalived, RabbitMQ metrics, then generate an anomaly-first markdown analysis report.
version: 1.1.0
user-invocable: true
metadata:
  openclaw:
...
当用户要求查看节点指标、分析系统资源、检查 OpenStack/Ceph/Keepalived/RabbitMQ 状态时，执行本 Skill。  
默认连接 `http://192.168.88.128:8428`，**可通过 `--url` 参数在调用时指定任意 VictoriaMetrics 地址**。  
通过 Prometheus 兼容 API 采集 **node、openstack、ceph、keepalived、rabbitmq** 等指标，脚本**自动探测**已接入的类别，并以 **Markdown 格式**输出异常优先的分析报告。
...
- Workspace：`<workspace>/.qoder/skills/vm-metrics-analysis/scripts/fetch_vm_metrics.py`
- 全局：`~/.openclaw/skills/vm-metrics-analysis/scripts/fetch_vm_metrics.py`
...
```bash
python3 <skill_dir>/scripts/fetch_vm_metrics.py --url {VM_URL} --range-hours 1 --step 60 --output text
```
...
| `--category` | `auto` | 指定采集类别：`node,openstack,ceph,keepalived,rabbitmq` 或 `auto`（自动探测） |

示例（指定地址和较长趋势窗口）：
```bash
python3 <skill_dir>/scripts/fetch_vm_metrics.py --url http://10.0.0.5:8428 --range-hours 6 --output text
```

> 脚本会自动探测 VictoriaMetrics 中存在的指标类别（node/openstack/ceph/keepalived/**rabbitmq**），无需手动指定。
...
**覆盖类别**：{node / keepalived / ceph / openstack / rabbitmq（仅列出已采集的）}  
**节点数量**：{N} 个实例  
...
- **告警**：{alarm_memory/alarm_disk/alarm_fd = 1 则为紧急告警}
- **队列积压**：{messages_unacked 或 messages_paged_out 异常高}
- **内存/磁盘**：{内存使用率近 watermark 、磁盘空间不足}
- **连接状态**：{连接数骤增 / 懒慧等}
- **建议**：{处理步骤}

> 若 RabbitMQ 无 alarm、messages_unacked < 1000、资源使用 < 80%，跳过本节。
...
`references/` 目录内存放了各组件的 Prometheus 指标样本，可在新增采集指标或排查 PromQL 时按需读取：

| 文件 | 内容 |
|---|---|
| `references/keepalived.txt` | keepalived_exporter 的完整 metric 列表（189 行） |
| `references/rabbitmq.txt` | RabbitMQ 内置 prometheus 端点 metric 列表（1102 行） |
| `references/ceph.txt` | ceph-mgr prometheus 模块 metric 列表（2328 行） |
| `references/openstack.txt` | openstack-exporter metric 列表（~2000 行） |
| `references/mysql.txt` | MySQL exporter metric 列表（备用） |

> 这些文件为实际采集输出的原始样本，可用于确认 metric 名、单位和标签。文件较大，**不建议全量加载到 context**，建议用 grep 搜索指定指标名。
...
- **章节裁剪**：Keepalived/Ceph/OpenStack/RabbitMQ 各组件只在有异常时输出对应小节，否则整节省略。
- **节点表格行省略**：第三章节点详情表中，✅ 的行可不写，只输出 ⚠️ 和 ❌ 的行。
- **告警阈値参考**：CPU > 85% 为⚠️，> 95% 为🔴；内存可用 < 20% 为⚠️，< 10% 为🔴；磁盘使用 > 80% 为⚠️，> 90% 为🔴；Load1 持续 > CPU 核数 × 0.8 为⚠️；RabbitMQ alarm 字段 = 1 为🔴，messages_unacked > 10000 为⚠️。
